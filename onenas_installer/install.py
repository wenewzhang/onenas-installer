...
async def create_one_pool(devices):
    ...
    await run(["zfs", "create", "-o", "mountpoint=/var/log", "-o", "canmount=on", f"{ONE_POOL}/log"])
    ...
    await run(["zfs", "set", "recordsize=128K", f"{ONE_POOL}/log"])
    await run(["zfs", "set", "xattr=sa", f"{ONE_POOL}/log"])
